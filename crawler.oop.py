...
class Web:
    ...
    def make_urls(self):
        """ URLS """
        self.url_sales_ninc_eps = (
            "http://www.marketwatch.com/investing/stock/" + self.stock + "/financials/"
        )
        self.url_roic = (
        ...
        self.url_ni_ttm = (
            "https://ycharts.com/companies/" + self.stock + "/net_income_ttm"
        )
        self.url_pe_ttm = "https://www.gurufocus.com/stock/" + self.stock
        self.url_rev_ttm = (
            "https://ycharts.com/companies/" + self.stock + "/revenues_ttm"
        )

    def parse_args(self, argv=None):
        parser = argparse.ArgumentParser()
        parser.add_argument("-s", "--stock")
        parser.description = "Crawl and Index Stock Data"
        parser.epilog = "Example: " + sys.argv[0] + " -s AAPL"
    ...
    def get_web_data(self):
        """ Get Web Data """
        print("Retrieving HTML for ", self.stock)
        r_sales_ninc_eps = self.err_web(self.url_sales_ninc_eps)
        r_roic = self.err_web(self.url_roic)
        r_url_fcf = self.err_web(self.url_fcf)
        r_bvps = self.err_web(self.url_bvps)
        r_ni_ttm = self.err_web(self.url_ni_ttm)
        r_pe_ttm = self.err_web(self.url_pe_ttm)
        r_rev_ttm = self.err_web(self.url_rev_ttm)
    ...
    def make_soup(
        self, r_sales_ninc_eps, r_url_fcf, r_bvps, r_roic, r_ni_ttm, r_pe_ttm, r_rev_ttm
    ):
        """ Soup Setup """
        print("Parsing HTML")
    ...
    def calc_growth(self, last, first, period):
        """ Simple cagr calculation """
        return (last / first) ** (1 / period) - 1

    def calc_cagr(self, years, data):
        ...
        return growth.apply(self.prettify_num)
    ...
    def check_data(self, data):
        ...
        if data is None:
            ...
            if data_is_valid:
                ...
                if denom:
                    self.denom_val = denom
                else:
                    self.denom_val = self.mynan

                if braces:
                    neg_pat = re.compile("-")
                    is_neg_already = neg_pat.search(num)
                    if is_neg_already:
                        return float(num), self.denom_val
                    else:
                        return -float(num), self.denom_val
                else:
                    return float(num), self.denom_val
            else:
                return self.mynan, self.mynan

    def check_growth_rate(self, data_name, data_master, denom_master, years):
    ...
    def prettify_num(self, num):
        """ Pretty print Growth Rate """
        return "%.2f" % (num * 100) + "%"

    def get_years_rev_ninc_eps(self, soup_sales_ninc_eps):
    ...
    def get_rev(self, soup_sales_ninc_eps, years_rev_ninc_eps):
        """ Revenue """
        rev_rn1 = "0"
        revenue = True
        revenue_master = []
        ...
        try:
            ...
            if sales_data_links is not None:
                for link in sales_data_links:
                    rev_val = link.string
                    rev, denom_val = self.check_data(rev_val)
                    revenue_master.append(rev)
                    revenue_denom_master.append(denom_val)

            revenue, revdf, rev_rn1 = self.check_growth_rate(
                "revenue", revenue_master, revenue_denom_master, years_rev_ninc_eps
            )
            self.summarize("Revenue", revdf)
            mg_rev, mg_rev_gr = self.mongoize_df(revdf, "revenue", denoms="yes")
            return revenue, revdf["Millions"], rev_rn1, mg_rev, mg_rev_gr

    def get_ninc(self, soup_sales_ninc_eps, years_rev_ninc_eps):
        """ Net Income """
        net_inc_rn1 = "0"
        net_inc = True
        net_inc_master = []
        ...
        try:
            ...
            for link in net_inc_values:
                ...
                safe, denom_val = self.check_data(net_income_val)
                net_inc_master.append(safe)
                net_inc_denom_master.append(denom_val)

            net_inc, net_inc_df, net_inc_rn1 = self.check_growth_rate(
                "Net Income", net_inc_master, net_inc_denom_master, years_rev_ninc_eps
            )
            self.summarize("Net Income", net_inc_df)
            mg_inc, mg_inc_line = self.mongoize_df(
                net_inc_df, "Net Income", denoms="yes"
            )
            return net_inc, net_inc_df["Millions"], net_inc_rn1, mg_inc, mg_inc_line

    def get_eps(self, soup_sales_ninc_eps, years_rev_ninc_eps):
        """ EPS """
        eps_rn1 = "0"
        eps_master = []
        eps_denom_master = []
        ...
        try:
            ...
            if eps_data is None:
                ...
                for tag in eps_data:
                    eps_val = tag.string
                    safe, denom_val = self.check_data(eps_val)
                    eps_master.append(safe)
                    # Expected to be 'None's
                    eps_denom_master.append(denom_val)

                eps, eps_df, esp_rn1 = self.check_growth_rate(
                    "EPS", eps_master, eps_denom_master, years_rev_ninc_eps
                )

                self.summarize("EPS", eps_df)
                mgeps_line, mgeps_gr_line = self.mongoize_df(eps_df, "EPS")
    ...
    def mongoize_df(self, df, name, denoms="no"):
    ...
    def get_fcf(self, soup_fcf):
        ...
        try:
            ...
            for tag in fcf_data:
                fcf_val = tag.string
                safe, denom_val = self.check_data(fcf_val)
            ...
            fcf, fcf_df, fcf_rn1 = self.check_growth_rate(
                "FCF", fcf_master, fcf_denom_master, years_fcf
            )
            self.summarize("FCF", fcf_df)
            mg_fcf, mg_fcf_gr = self.mongoize_df(fcf_df, "FCF", denoms="yes")
            return fcf, fcf_df["Millions"], years_fcf, fcf_rn1, mg_fcf, mg_fcf_gr

    def get_bvps(self, soup_bvps):
        """ BVPS """
        bvps = True
        bvps_rn1 = "0"
        years_bvps = []
        ...
        try:
            ...
            for tag in bvps_data_in_links:
                ...
                if bvps_val is not None:
                    ...
                    safe, denom_val = self.check_data(bvps_data)
    ...
    def get_roic(self, soup_roic):
        ...
        try:
            ...
            print(self.stock, "had", roic, "ROIC")
            mg_roic = f"'ROIC':'{roic}'"
            return mg_roic

    def get_ni_ttm(self, soup_ni_ttm):
        """ Net Income TTM """
        ...
        try:
            ...
            ni_ttm, ni_ttm_denom = self.check_data(net_inc_ttm_data)
            print(self.stock, "had", ni_ttm, ni_ttm_denom, "NetInc TTM")
            mg_ni_ttm = (
                f"'NetIncTTM' : {{ 'Val' : {ni_ttm}, 'Den' : '{ni_ttm_denom}' }}"
            )
            return ni_ttm, mg_ni_ttm

    def get_pe_ttm(self, soup_pe_ttm):
        wanted_text = "P/E"
        ...
        if pe_ttm:
            for index, div_tag in enumerate(pe_ttm):
                if wanted_text in str(pe_ttm[index]):
                    ...
                    print(f"{self.stock} PE: {pe_ttm_num}")
                    return f"{self.stock} PE: {pe_ttm_num}"
        else:
            print("No PE TTM data found for " + stock)
            return f"{self.stock} PE: 'NA'"

    def get_mcap(self, soup_pe_ttm):
        ...
        for index, div_tag in enumerate(market_cap):
            if wanted_text in str(market_cap[index]):
                ...
                print(f"{self.stock} Market Cap: {market_cap_text}")
                mc_val, mc_den = market_cap_text.strip().split(" ")
                mg_mcap = f"'MC' : {{'Val' : {mc_val} , 'Den' : '{mc_den}' }}"
                return mg_mcap

    def get_rev_ttm(self, soup_rev_ttm):
        ...
        try:
            ...
            rev_ttm, rev_ttm_denom = self.check_data(rev_ttm_data)
            try:
                int(rev_ttm)
            except ValueError:
                rev_ttm = "NA"
                ...
                mg_rev_ttm = (
                    f"'RevTTM' : {{'Val' : {rev_ttm} , 'Den' : '{rev_ttm_denom}' }}"
                )
                # print ('N',mg_rev_ttm)
            print(
                self.stock
    ...
    def main(self):
        ...
        years_rev_ninc_eps = self.get_years_rev_ninc_eps(soup_sales_ninc_eps)
        revenue, revenue_master, rev_rn1, mg_rev, mg_rev_gr = self.get_rev(
            soup_sales_ninc_eps, years_rev_ninc_eps
        )

        net_inc, net_inc_master, net_inc_rn1, mg_inc, mg_inc_line = self.get_ninc(
            soup_sales_ninc_eps, years_rev_ninc_eps
        )

        eps, eps_master, eps_rn1, mgeps_line, mgeps_gr_line = self.get_eps(
            soup_sales_ninc_eps, years_rev_ninc_eps
        )
        fcf, fcf_master, years_fcf, fcf_rn1, mg_fcf, mg_fcf_gr = self.get_fcf(soup_fcf)
        bvps, bvps_master, years_bvps, bvp_rn1 = self.get_bvps(soup_bvps)
        mg_roic = self.get_roic(soup_roic)
        ni_ttm, mg_ni_ttm = self.get_ni_ttm(soup_ni_ttm)
        pe_ttm = self.get_pe_ttm(soup_pe_ttm)
        mg_rev_ttm = self.get_rev_ttm(soup_rev_ttm)
        mg_mcap = self.get_mcap(soup_pe_ttm)
        ...
        for x in [
            ...
            mongo_temp.append(x)
        mongo_doc = ",".join(mongo_temp)
        mongo_doc = f"{{ 'Stock':'{self.stock}', {mongo_doc} }}"
